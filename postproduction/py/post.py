...
def create_post(episode_name, episode_number=None):
    ...
    '''
    logger = make_logger(episode_name)
    ...
    write_log(logger, title, header_html, references_html)
...
def write_log(logger, title, header_html, references_html):
    '''
    write log
    Args:
        logger: logger
...
def make_logger(episode_name, episode_number=None):
...
def search_episodes(attrs):
    ...
    for f in files:
        is_valid = False
        d = read_json(f)
        if (d['Number'].split('-')[0] in ['hwn', 'football', 'weshow'] and d['Number'].split('-')[0] == attrs['title']):
            is_valid = True
            count += 1
